...
<div class="bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
    <div class="px-6 py-4 border-b border-border">
        <h2 class="text-lg font-bold text-dark font-display">Edit Berita</h2>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                    @error('title') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="content" class="block text-sm font-medium text-dark mb-1">Isi Berita</label>
                    <textarea name="content" id="content" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 h-96">{{ old('content', $post->content) }}</textarea>
                ...
                <div class="pt-4 border-t border-border">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $post->meta_title) }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                        </div>
                        <div>
                            <label for="meta_description" class="block text-sm font-medium text-dark mb-1">Meta Description</label>
                            <textarea name="meta_description" id="meta_description" rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">{{ old('meta_description', $post->meta_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-gray-50 rounded-xl p-5 border border-border">
                    ...
                    <div class="mb-4">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
                ...
                <div class="bg-gray-50 rounded-xl p-5 border border-border">
                    ...
                    @if($post->thumbnail)
                        <div class="mb-3">
                            <img src="{{ Storage::url($post->thumbnail) }}" alt="Thumbnail" class="w-full h-auto rounded-lg">
                        </div>
                    @endif
                    ...
                    <p class="mt-2 text-xs text-gray-500">💡 <strong>Saran Ukuran:</strong> Gunakan gambar dengan rasio 4:3 (contoh: 800x600 px) agar tampilan tetap rapi saat dilihat pengunjung.</p>

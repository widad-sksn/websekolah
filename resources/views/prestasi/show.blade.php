...
@section('meta')
<title>{{ $achievement->title }} - {{ config('app.name', 'SchoolCMS') }}</title>
<meta name="description" content="{{ Str::limit(strip_tags($achievement->description ?? 'Prestasi ' . $achievement->title), 150) }}">
<!-- Open Graph -->
<meta property="og:title" content="{{ $achievement->title }}">
<meta property="og:description" content="{{ Str::limit(strip_tags($achievement->description ?? 'Prestasi ' . $achievement->title), 150) }}">
<meta property="og:image" content="{{ asset('storage/' . $achievement->photo) }}">
...
<section class="bg-slate-900 pt-20 pb-24 md:pt-24 md:pb-32 relative overflow-hidden">
    <div class="absolute inset-0">
        ...
        <img src="{{ Storage::url($achievement->photo) }}" alt="{{ $achievement->title }}" class="w-full h-full object-cover opacity-20 blur-sm">
    ...
    <div class="container mx-auto px-4 relative z-10 max-w-4xl text-center">
        <a href="/prestasi" class="inline-block px-3 py-1 md:px-4 md:py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs md:text-sm font-semibold rounded-full shadow-lg transition-colors mb-4 md:mb-6">
            Prestasi {{ $achievement->category }}
        </a>
        <h1 class="text-2xl md:text-5xl font-bold font-display text-white leading-tight mb-4 md:mb-6">{{ $achievement->title }}</h1>
        <div class="flex flex-wrap items-center justify-center text-xs md:text-sm text-slate-300 gap-3 md:gap-6">
            <span class="flex items-center"><svg class="w-4 h-4 md:w-5 md:h-5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg> {{ $achievement->level }}</span>
            <span class="flex items-center"><svg class="w-4 h-4 md:w-5 md:h-5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> Tahun {{ $achievement->year }}</span>
        </div>
    </div>
</section>
        ...
        <div class="flex flex-col lg:flex-row gap-8 {{ $recent_achievements->count() == 0 ? 'justify-center' : '' }}">
                ...
                <article class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                    ...
                    <div class="px-8 md:px-12 py-6 bg-gray-50 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($achievement->title) }}" target="_blank" class="w-10 h-10 rounded-full bg-sky-500 text-white flex items-center justify-center hover:bg-sky-600 transition-colors shadow-sm">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"></path></svg>
                            </a>
                            <a href="https://wa.me/?text={{ urlencode($achievement->title . ' ' . request()->url()) }}" target="_blank" class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition-colors shadow-sm">
            ...
            <div class="{{ $recent_achievements->count() > 0 ? 'lg:w-1/4 space-y-8' : 'hidden' }}">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="text-lg font-bold font-display text-dark mb-6">Prestasi Lainnya</h3>
                    <div class="space-y-6">
                        @foreach($recent_achievements as $recent)
                        <div class="flex gap-4 group">
                            <a href="/prestasi/{{ $recent->id }}" class="w-20 h-20 flex-shrink-0 rounded-xl overflow-hidden">
                                <img src="{{ Storage::url($recent->photo) }}" alt="{{ $recent->title }}" loading="lazy" class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-300">
                            </a>
                            <div>
                                <h4 class="font-bold text-sm text-dark leading-tight group-hover:text-blue-600 transition-colors">
                                    <a href="/prestasi/{{ $recent->id }}">{{ Str::limit($recent->title, 45) }}</a>
                                </h4>
